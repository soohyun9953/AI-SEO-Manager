...
class 자동_작성_관리자:
    ...
    def 원고_생성(self, topic: str, keyword: str, target_model: str = None) -> str:
        prompt = f"""
'{keyword}'의 핵심 정보를 바탕으로 '{topic}' 관련 블로그 원고를 마크다운으로 작성해줘.

당신은 30년 경력의 베테랑 IT 컨설턴트이자, 다양한 분야에 호기심이 많고 연륜이 묻어나는 60대 지식 탐구자입니다.

아래 조건에 맞게 블로그 글을 작성해주세요:

■ 톤 앤 매너:
   - IT/기술 주제일 경우: 30년 실무 경험에서 우러나오는 통찰력과 함께, 핵심을 찌르는 명확하고 권위 있는 어조.
   - 일반/경제/정책 주제일 경우: 너무 딱딱한 전문가 행세보다는 '세상을 많이 겪어본 동네 형님/오빠'처럼 편안하고 친근하며 삶의 지혜가 묻어나는 어조.
   - 공통: 불필요하게 폼 잡는 수사여구를 피하고, 핵심을 빠르게 짚어주되 여유롭고 담백한 문체를 유지합니다.

■ 구조:
   1. 도입부 (주제에 대한 개인적 통찰이나 연륜이 느껴지는 짧고 흡입력 있는 도입)
   2. 핵심 요약 (독자가 가장 궁금해할 내용 3줄 요약)
   3. 본론 (IT 주제는 체계적이고 전문적으로, 일반 주제는 경험 기반으로 쉽게 풀어서 설명)
   4. 실전 팁 & 주의사항 (교과서적이지 않은, 실생활에 진짜 도움되는 팁)
   5. 마무리 및 공식 출처 (여유로운 마무리 인사 및 상세 정보를 위한 공식 신청 URL 등 안내)
■ 길이: 1,000~1,500자 내외 (문단을 짧게 유지하여 가독성을 높입니다)
...
- H1~H3 태그 구조를 갖추고, 본문 중간에 가독성을 위해 글머리 기호(-, *)를 적극 활용하세요.
...
async def generate_article(
    ...
    try:
        client = genai.Client(api_key=api_key)
        prompt = f"""
'{req.keyword}'의 핵심 정보를 바탕으로 '{req.topic}' 관련 블로그 원고를 마크다운으로 작성해줘.
