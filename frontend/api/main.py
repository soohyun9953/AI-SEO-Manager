...
async def generate_article(req: ArticleRequest, x_gemini_key: Optional[str] = Header(None)):
    api_key = x_gemini_key or GEMINI_API_KEY
    client = genai.Client(api_key=api_key)
    prompt = f"[현재 시점: {datetime.now().strftime('%Y년 %m월 %d일')}] '{req.keyword}' 중심 '{req.topic}' 블로그 원고 작성. 당신은 해당 주제 분야의 30년 차 최고 권위자입니다. 단순 서술을 넘어 도입 목적(혜택), 활용 방법, 핵심 특징을 구조화하세요. 독자가 당장 적용하고 싶도록 설득력 있고 실용적인 가치를 강조하고, 분야 최고 전문가 수준의 심도 있는 통찰을 제공하세요. 주제와 키워드는 해당 도메인의 현실적인 맥락에서 해석하고 SF 등 허구적 상상은 배제하세요. 정책이나 지원금 관련 주제는 반드시 주관 정부 부처, 공식 기관명 및 공식 채널을 명시하여 공신력을 높이세요. 'SEO' 단어 절대 금지. 마지막에는 '주요 키워드: ' 문구와 함께 관련 키워드 5개를 쉼표(,)로 구분하여 추가해줘."
    try:
...
async def publish_tistory(req: PublishRequest, x_gemini_key: Optional[str] = Header(None)):
    api_key = x_gemini_key or GEMINI_API_KEY
    client = genai.Client(api_key=api_key)
    prompt = f"[현재 시점: {datetime.now().strftime('%Y년 %m월 %d일')}] '{req.keyword}' 중심 '{req.topic}' 원고. 해당 분야의 30년 차 최고 권위자로서 단순 서술을 넘어 목적, 활용 방법, 특징을 구조화하여 독자가 당장 적용해보고 싶도록 설득력 있게 작성할 것. 최고 수준의 심도 있는 통찰 필수. 주제와 키워드는 현실적/실무적 맥락에서 해석하고 SF적 상상은 배제할 것. 정책이나 지원금 관련 주제는 반드시 주관 정부 부처, 공식 기관명 및 공식 채널을 명시하여 공신력을 높일 것. 'SEO' 단어 금지. 마지막에 '주요 키워드: '와 함께 키워드 5개를 쉼표(,)로 구분하여 추가할 것."
    try:
        response = await safe_generate_content_async(client, prompt)
